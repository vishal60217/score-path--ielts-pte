...
if (hasPostgres) {
  ...
  pgPool = new Pool({ connectionString: process.env.DATABASE_URL, ssl: process.env.DATABASE_SSL === 'false' ? false : { rejectUnauthorized: false }, max: 3, connectionTimeoutMillis: 8000, idleTimeoutMillis: 10000, keepAlive: true });
}
...
app.get('/api/health',async(q,r)=>{
  ...
  r.status(healthy?200:503).json({ok:healthy,service:'ScorePath',database:hasPostgres?'postgres':'local',productionReady:hasPostgres&&healthy,error:startupError?(process.env.NODE_ENV==='production'?'Database initialization failed. Check DATABASE_URL and Vercel logs.':startupError.message):null});
});
